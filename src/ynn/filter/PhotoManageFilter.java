...
/**
 * Servlet Filter implementation class PhotoManageFilter
 */
@WebFilter("/PhotoManageFilter")
public class PhotoManageFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		System.out.println(choose+"照片选择");
		...
		try{
			...
			if(choose.length()!=0){
				if(choose.equals("0")){//进行查询
						String sql0="select time,photo from photo where username=?;";//这里得到的是照片的地址，具体显示还没有实现，待定
						ps=ct.prepareStatement(sql0);
						ps.setString(1, username);
						rs=ps.executeQuery();
						System.out.println("时间\t\t照片地址<br>");
						while(rs.next()){
						...
						String sql1="delete from photo where username = ?;";
